...
pub fn Chunk(comptime size: comptime_int, comptime height: comptime_int) type {
    return struct {
        blocks: [size][height][size]Block,
        ...
        pub fn init() @This() {
            ...
            for (0..size) |x| {
                for (0..height) |y| {
                    for (0..size) |z| {
                        const edge = x == 0 or x == size - 1 or z == 0 or z == size - 1;
                        c.blocks[x][y][z] = if (edge)
                            if (y < height - 1) .stone else .grass
        ...
        pub fn get(self: *const @This(), x: i32, y: i32, z: i32) Block {
            if (x < 0 or x >= size or y < 0 or y >= height or z < 0 or z >= size) return .air;
...
pub fn VoxelWorld(comptime ChunkType: type) type {
    return struct {
        chunk: ChunkType,

        pub fn init() @This() {
            return .{ .chunk = ChunkType.init() };
        }
                ...
                while (bx <= @as(i32, @intFromFloat(@floor(region.max.x())))) : (bx += 1) {
                    ...
                    while (by <= @as(i32, @intFromFloat(@floor(region.max.y())))) : (by += 1) {
                        ...
                        while (bz <= @as(i32, @intFromFloat(@floor(region.max.z())))) : (bz += 1) {
                            if (!self.chunk.solid(bx, by, bz)) continue;
        ...
        pub fn buildMesh(self: *const @This(), verts: []rend.Vertex, indices: []u16, comptime colors: fn (Block) [3]f32) struct { vcount: usize, icount: usize } {
            const T = @TypeOf(self.chunk.blocks);
            const size = @typeInfo(T).array.len;
            const height = @typeInfo(@typeInfo(T).array.child).array.len;
            const max_slice = @max(size * height, @max(size * size, height * size));

            var mask: [max_slice]bool = undefined;
            var vi: usize = 0;
            var ii: usize = 0;
            ...
            for (axes, 0..) |ax, dir| {
                const dim = [3]usize{ size, height, size };
                for (0..2) |back| {
                    const stp: i32 = if (back == 0) 1 else -1;
                    var d: i32 = if (back == 0) 0 else @intCast(dim[ax.d] - 1);

                    while ((back == 0 and d < dim[ax.d]) or (back == 1 and d >= 0)) : (d += stp) {
                        var n: usize = 0;
                        for (0..dim[ax.v]) |v| {
                            for (0..dim[ax.u]) |u| {
                                var pos = [3]i32{ 0, 0, 0 };
                                ...
                                const blk = self.chunk.get(pos[0], pos[1], pos[2]);
                                if (blk == .air) {
                                    mask[n] = false;
                                    n += 1;
                                    continue;
                                ...
                                pos[ax.d] += stp;
                                mask[n] = self.chunk.get(pos[0], pos[1], pos[2]) == .air;
                                n += 1;
                            }
                        }
                        ...
                        for (0..dim[ax.v]) |v| {
                            ...
                            while (u < dim[ax.u]) {
                                ...
                                while (u + w < dim[ax.u] and mask[n + w]) : (w += 1) {
                                    var p2 = pos;
                                    p2[ax.u] = @intCast(u + w);
                                    if (self.chunk.get(p2[0], p2[1], p2[2]) != blk) break;
                                }
                                ...
                                outer: while (v + h < dim[ax.v]) : (h += 1) {
                                    for (0..w) |k| {
                                        ...
                                        p2[ax.u] = @intCast(u + k);
                                        p2[ax.v] = @intCast(v + h);
                                        if (self.chunk.get(p2[0], p2[1], p2[2]) != blk) break :outer;
                                    }
                                }
