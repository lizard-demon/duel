...
pub const Player = struct {
    ...
    pub const Input = struct {
        ...
        pub const handle = struct {
            ...
            pub fn crouch(p: *Player, world_map: *Map, stand_height: f32, crouch_height: f32, width: f32) void {
                ...
                if (p.crouch and !wish) {
                    ...
                    const standing = Player.Input.lib.standbox(test_pos, width, stand_height);
            ...
            pub fn blocks(p: *Player, world_map: *Map, reach: f32, crouch_height: f32, stand_height: f32, width: f32) bool {
                ...
                const look = Player.Input.lib.lookdir(p.yaw, p.pitch);
                ...
                if (p.io.mouse.rightPressed()) {
                    ...
                    const player_box = Player.Input.lib.bbox(p.pos, p.crouch, crouch_height, stand_height, width);
                    const block_box = Player.Input.lib.blockbox(block_pos);
        ...
        pub fn tick(player_ptr: *Player, world_map: *Map, dt: f32, cfg: Config, game_cfg: anytype) bool {
            Player.Input.handle.movement(player_ptr, player_ptr.yaw, dt, game_cfg);
            Player.Input.handle.crouch(player_ptr, world_map, cfg.stand_height, cfg.crouch_height, cfg.width);
            Player.Input.handle.jump(player_ptr, cfg.jump_power);
            Player.Input.handle.camera(player_ptr, cfg.sensitivity, cfg.pitch_limit);
            const world_changed = Player.Input.handle.blocks(player_ptr, world_map, cfg.reach, cfg.crouch_height, cfg.stand_height, cfg.width);
            Player.Input.handle.color(player_ptr);
            Player.Input.handle.mouse(player_ptr);
